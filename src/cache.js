...
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_PUBLIC_KEY || ''; // Needs a key to read
...
export const syncSlackUsers = async () => {
    ...
    try {
        ...
        const validUsers = slackUsers.filter(u => !u.is_bot && !u.deleted && u.id !== 'USLACKBOT');

        const { data: dbMembers, error } = await supabase.from("execs").select("id, slack_user_id, name");
        if (error) throw error;
        ...
        for (const u of validUsers) {
            ...
            if (dbMap.has(slackId)) {
                ...
                if (dbUser.name !== name) {
                    const { error: err } = await supabase.from("execs").update({ name }).eq("id", dbUser.id);
                    if (err) throw new Error(`Supabase Update Error: ${err.message}`);
                    // TODO: Remove debug log
                    console.log(`Updated name for ${slackId}: from ${dbUser.name} to ${name}`);
                }
            } else {
